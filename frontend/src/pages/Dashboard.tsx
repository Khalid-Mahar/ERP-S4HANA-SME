...
import { salesApi, inventoryApi, api } from '../api';
...
const WidgetRenderer = ({ widget }: { widget: any }) => {
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.get<any>(widget.endpoint),
  });

  if (widget.type === 'stat_card') {
    return (
      <div className={`card group hover:border-indigo-500 transition-all duration-300 ${widget.grid_size}`}>
        <div className="flex items-start justify-between">
          <div className={`p-3 rounded-2xl bg-${widget.color}-500 text-white shadow-lg`}>
            {widget.id.includes('profit') ? <TrendingUp size={20} /> : <DollarSign size={20} />}
          </div>
        </div>
        <div className="mt-4">
          <p className="text-xs font-bold text-[#64748b] uppercase tracking-widest">{widget.title}</p>
          <h3 className="text-2xl font-black text-[#0f172a] mt-1">
            {isLoading ? '...' : `$${Number((data as any)?.value || (data as any)?.totalValuation || 0).toLocaleString()}`}
          </h3>
        </div>
      </div>
    );
  ...
  if (widget.type === 'area_chart' || widget.type === 'bar_chart') {
    return (
      <div className={`card !p-0 overflow-hidden ${widget.grid_size}`}>
        <div className="px-6 py-4 border-b border-[#e2e8f0] flex items-center justify-between">
          <h3 className="text-sm font-black text-[#0f172a] uppercase tracking-wider">{widget.title}</h3>
          <BarChart3 size={16} className="text-[#64748b]" />
        </div>
        <div className="p-6 h-[200px] flex items-center justify-center text-[#64748b] bg-gray-50/50">
          <div className="text-center">
            <p className="text-xs font-bold italic">Analytics data visualization pending Chart.js</p>
          </div>
        </div>
      </div>
    );
...
export default function Dashboard() {
  ...
  const { data: lowStock } = useQuery({
    ...
    enabled: user?.role === 'ADMIN' || user?.role === 'WAREHOUSE_HEAD',
  });
  ...
  return (
    <div className="space-y-8">
      {/* Dynamic Widgets */}
      {roleConfig.widgets && roleConfig.widgets.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {roleConfig.widgets.map((w: any) => <WidgetRenderer key={w.id} widget={w} />)}
        </div>
      )}

      {/* Default Stats for Admins/Managers */}
      {(user?.role === 'ADMIN' || !roleConfig.widgets?.length) && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {defaultStats.map((stat, i) => <StatCard key={i} {...stat} />)}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* Inventory Alerts (Only for Warehouse/Admin) */}
        {(user?.role === 'ADMIN' || user?.role === 'WAREHOUSE_HEAD') && (
